...
> SPCM is only the name of their Python API I believe so this repo's name is wrong, but ``spectrum-api-rs`` sounded lame.
...
Those utils use no ``unsafe`` blocks apart from individual calls to the SDK's functions. Also they use standard Rust naming convention, as opposed to the SDK's functions (no shot you'll ever catch me using systems hungarian notation).
